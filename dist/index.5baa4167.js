...
function renderTransactions() {
    (0, _firestore.onSnapshot)((0, _firestore.collection)((0, _firebaseJs.db), 'transactions'), (snapshot)=>{
        let income = 0;
        let expenses = 0;
...
window.editTransaction = (id, description, amount, category)=>{
    ...
    if (newDescription && !isNaN(newAmount) && newCategory) updateTransaction(id, newDescription, newAmount, newCategory);
    else alert("Invalid input. Please try again.");
};
async function updateTransaction(id, description, amount, category) {
    const transactionRef = (0, _firestore.doc)((0, _firebaseJs.db), 'transactions', id);
    try {
        await (0, _firestore.updateDoc)(transactionRef, {
            description,
            amount,
            category,
            date: new Date().toISOString()
...
window.deleteTransaction = async (id)=>{
    if (confirm("Are you sure you want to delete this transaction?")) try {
        await (0, _firestore.deleteDoc)((0, _firestore.doc)((0, _firebaseJs.db), 'transactions', id));
